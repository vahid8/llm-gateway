...
- **Fallback chains** (`FALLBACK_CHAINS`, JSON): server-side default fallbacks in `routing._candidates`, applied **only when the request carries no `fallback_models`** (client list wins outright, it is not merged). Lookup is on the *normalized* primary: exact model key first, then the `provider/*` wildcard. Embeddings use `wildcard=False` so a provider-wide chat chain can't route an embedding to a chat model — give embedding models their own exact entries. Candidates are deduped on normalized form.
